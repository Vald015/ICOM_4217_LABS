...
int fixedIndex1(int indexA);
int fixedIndex2(int indexB);
...
int main(void)
{
	...
	while(1){
		...
		while(!(GPIOPinRead(GPIO_PORTA_BASE, GPIO_PIN_5))){
			...
			if (ui8LED == 16)//To Turn ON send a logic 1 - (LED) Pull Down
			{ui8LED = 0;} //if LED was ON turn it OFF
			else {ui8LED = 16;}// if LED was OFF turn it ON

			GPIOPinWrite(GPIO_PORTA_BASE, GPIO_PIN_4, ui8LED); //toggle LED

			index = index +2;//increment index
			index2 = index2 +2;//increment second word index
			index = checkIndex(index);//index
			index2 = checkIndex(index2);//check index2
			index2 = fixedIndex2(index2);
		...
		while((GPIOPinRead(GPIO_PORTA_BASE, GPIO_PIN_7))){
			...
			if (ui8LED2 == 0)//To turn ON send a logic 0 - Pull Up (LED Pull Up)
			{ui8LED2 = 64;}//if LEd is ON turn it OFF
			else {ui8LED2 = 0;}// if LED is OFF turn it ON

			GPIOPinWrite(GPIO_PORTA_BASE, GPIO_PIN_6, ui8LED2);//toggle LED

			index = index-2;//decrement index
			index2 = index2 -2;//decrement index2
			index = checkIndex(index);//index
			index2 = checkIndex(index2);//check index2
			index = fixedIndex1(index);
